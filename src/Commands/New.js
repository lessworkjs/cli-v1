...
class New extends BaseCommand {
   ...
   */
  get description() {
    return 'Scaffold a new Lesswork application with the name provided.'
  }
  ...
  * handle(args, options) {
    ...
    this.blueprint = options.blueprint || 'lessworkjs/lesswork'
  ...
  * _cloneRepository() {
    ...
    this._startSpinner(
      this.colors.blue(`Cloning ${branch} branch of ${blueprint}`)
    )
  ...
  * _copyEnvironmentFile() {
    try {
      yield pify(fs.copy)(
        path.join(this.applicationPath, '.env.example.yml'),
        path.join(this.applicationPath, '.env.yml')
      )
  ...
  * _generateSecureKey() {
    try {
      yield pify(exec)('node work key:generate')
