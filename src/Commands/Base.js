...
class Base extends Command {
  ...
  * _checkRequirements() {
    ...
    if (semver.satisfies(nodeVersion.substring(1), '> 6.11.0')) {
      this.error(`${this.icon('error')} Your current Node.js version doesn't match Lesswork requirements.`)
      this.error(`${this.icon('info')} Please update your Node.js installation to <= 6.10 before continue.`)
      error = true
    }

    if (!semver.satisfies(npmVersion, '>=3.0.0')) {
      this.error(`${this.icon('error')} Your current npm version doesn't match Lesswork requirements.`)
    ...
    this.success(`${this.icon('success')} Your current Node.js & npm version match the Lesswork requirements!`)
  }
